...
gff_file = '/mnt/Lovelace/data/tracks/hg19/gencode.v27lift37.annotation.gff3.gz'
peak_file  = c('K562' = '/mnt/Lovelace/data/tracks/hg19/SuRE_elasticNet_peaks_K562.bed',
               'HEPG2' = '/mnt/Lovelace/data/tracks/hg19/SuRE_elasticNet_peaks_K562.bed')
txdb_file = '/mnt/Lovelace/projects/SuRE/SuRE_K562/txdb.gencode.v27lift37.sqlite'

tss_file = '/mnt/Lovelace/projects/SuRE/SuRE_K562/tss_matrix.txt.gz'
lookup_matrix = '/mnt/Lovelace/projects/SuRE/SuRE_K562/lookup_matrix.txt.gz'

offset_coefs = readRDS("/mnt/Lovelace/projects/SuRE/SuRE_K562/bins_vince/optimalFitOffsetCoefficients_K562_spatial.rds")
chrom_vec = paste0('chr', c(1:22, 'X', 'Y'))

jbrowse = "https://rhpc.nki.nl/lovelace/c.leemans/jbrowse-hg19/index.html?tracklist=0&nav=0&overview=0"
...
hg38ToHg19 = "/mnt/Lovelace/data/hg38ToHg19.over.chain"
hg19ToHg38 = "/mnt/Lovelace/data/hg19ToHg38.over.chain"
...
init <- function(gff_file, txdb, rd_file, chrom_vec){
  if (!file.exists(rd_file)){
    symbol_dt = unique(fread(cmd=paste("zcat", gff_file, "| grep -P '\\tgene\\t' |",
                                       "sed -r 's/gene_[in][^=]+=([^;]*);/\\t\\1\\t/g' |",
                                       "sed -r 's/_[0-9]+//' | awk '{print $10, $12}'"),
                             stringsAsFactors=F, col.names=c('gene_id', 'gene_name'),
                             key = 'gene_name'))

    tss_gr = promoters(txdb, columns=c('gene_id', 'tx_name'),
                       upstream=0, downstream=1,
                       filter=list(tx_chrom=chrom_vec))
    tss_gr$gene_id = unlist(tss_gr$gene_id)
    tss_gr <- c(sort(tss_gr[strand(tss_gr) == "+"]),sort(tss_gr[strand(tss_gr) == "-"]))
    regions <- resize(tss_gr, 501, fix = 'center')


    suppressWarnings(glm_plus <-import("/mnt/Lovelace/data/tracks/hg19/SURE_elasticNet_allele_K562_plus.bw",
                                       selection = BigWigSelection(regions[strand(regions) == "+"]),
                                       as = "NumericList"))

    suppressWarnings(glm_minus <-import("/mnt/Lovelace/data/tracks/hg19/SURE_elasticNet_allele_K562_minus.bw",
                                        selection = BigWigSelection(regions[strand(regions) == "-"]),
                                        as = "NumericList"))

    max_coef <- max(c(glm_plus, glm_minus))
    max_tss <- tapply(1:length(max_coef), unlist(tss_gr$gene_id),
                      function(n) n[which.max(max_coef[n])])
    tss_gr$max_coef <- max_coef
    tss <- tss_gr[as.integer(max_tss)]

    id_dt = suppressWarnings(as.data.table(tss, key='gene_id'))
    symbol_dt[,transcript_id := id_dt[gene_id,'tx_name']]

    ensembl_dt = id_dt[,list(transcript_id = tx_name,
                             ensembl_id = gsub('[.]*', '', gene_id))]
    setkey(ensembl_dt, 'ensembl_id')

    gencode_dt = id_dt[,c('gene_id', 'tx_name')]
    colnames(gencode_dt) = c('gene_id', 'transcript_id')

    transcript_dt = data.table(transcript_id = names(tss_gr),
                               ensembl_tid = gsub('[.]*', '', names(tss_gr)),
                               key='ensembl_tid')
  ...
  return(list(lookup_list=lookup_list, tss_gr=tss_gr))
}



init(gff_file, txdb, rd_file, chrom_vec)
...
triangle_dt <- function(center, upstream = 1000, downstream=1000,
                        ...
                        path = "/mnt/Lovelace/data/tracks/hg19/",
...
server <- function(input, output, session) {
  ...
  # 2. Its output type is a plot
    gff_file = '/mnt/Lovelace/data/tracks/hg19/gencode.v27lift37.annotation.gff3.gz'
    txdb_file = '/mnt/Lovelace/projects/SuRE/SuRE_K562/txdb.gencode.v27lift37.sqlite'
    rd_file = '/mnt/Lovelace/projects/SuRE/SuRE_K562/promoter_triangle_input.RData'
    chrom_vec = paste0('chr', c(1:22, 'X', 'Y'))
    ...
    init_list = init(gff_file, txdb, rd_file, chrom_vec)
    ...
    dataInput <- eventReactive(input$go, {
        center = get_center(input$ROI, init_list$lookup_list, init_list$tss_gr)
        vals$center = center
